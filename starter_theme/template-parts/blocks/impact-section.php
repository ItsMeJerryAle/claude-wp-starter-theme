 ...
 * ACF fields: impact_eyebrow, impact_heading, impact_stories_label/url,
 *             impact_stat1/2/3_number/label/desc
 * Data: Testimonial CPT posts via WP_Query
 */
        ...
        <div class="md:hidden">
            ...
            <div class="flex items-center justify-between mt-6">
                <div class="flex items-center gap-3">
                    <button id="impact-prev" aria-label="<?php esc_attr_e( 'Previous testimonial', 'starter_theme' ); ?>"
                    ...
                    <button id="impact-next" aria-label="<?php esc_attr_e( 'Next testimonial', 'starter_theme' ); ?>"
                            class="w-12 h-12 rounded-full bg-primary text-secondary flex items-center justify-center hover:brightness-110 transition-all">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
            ...
            <div data-animate data-delay="0"
                ...
                ?>
                    <div class="border-t border-primary px-8 pt-8">
                        <p class="font-barlow-condensed font-semibold leading-none tracking-tight text-leaf" style="font-size: 88px;">
...
(function () {
    ...
    var pos = 1, busy = false;
    function getSlideW() { return Math.round( viewport.offsetWidth * 0.85 ) + 15; }
    function getOffset() { return pos * ( getSlideW() + GAP ); }
    function realIdx( i ) { if ( i === 0 ) return realCount - 1; if ( i === realCount + 1 ) return 0; return i - 1; }
    function initSizes() { var w = getSlideW(); allSlides.forEach( function(s) { s.style.width = w + 'px'; } ); }
    function applyTranslate( animated ) { track.style.transition = animated ? 'transform 0.5s ease-out' : 'none'; track.style.transform = 'translateX(-' + getOffset() + 'px)'; if ( ! animated ) track.offsetHeight; }
    function updateStyles() {
        var activeReal = realIdx( pos );
        allSlides.forEach( function( slide, i ) {
            var bg = slide.querySelector('.slide-bg'), grad = slide.querySelector('.slide-gradient'), solid = slide.querySelector('.slide-solid'), quote = slide.querySelector('.slide-quote');
            if ( realIdx(i) === activeReal ) { if (slide.dataset.bg) bg.style.backgroundImage = "url('"+slide.dataset.bg+"')"; grad.style.display='block'; solid.style.display='none'; if(quote){quote.style.maxWidth='33rem';quote.style.opacity='1';} }
            else { bg.style.backgroundImage='none'; grad.style.display='none'; solid.style.display='block'; if(quote){quote.style.maxWidth='20rem';quote.style.opacity='0.8';} }
        });
    }
    function goTo(dir) { if(busy)return; busy=true; pos+=dir; updateStyles(); applyTranslate(true); }
    track.addEventListener('transitionend', function(e) {
        if(e.propertyName!=='transform')return; busy=false;
        if(pos===0||pos===realCount+1) { pos=(pos===0)?realCount:1; applyTranslate(false); requestAnimationFrame(function(){requestAnimationFrame(function(){track.style.transition='transform 0.5s ease-out';});}); }
    });
    initSizes(); applyTranslate(false); updateStyles();
    requestAnimationFrame(function(){requestAnimationFrame(function(){track.style.transition='transform 0.5s ease-out';});});
    ...
    viewport.addEventListener('touchstart',function(e){touchStartX=e.touches[0].clientX;touchStartY=e.touches[0].clientY;isDragging=false;},{passive:true});
    viewport.addEventListener('touchmove',function(e){var dx=e.touches[0].clientX-touchStartX,dy=e.touches[0].clientY-touchStartY;if(!isDragging&&Math.abs(dx)<Math.abs(dy))return;isDragging=true;e.preventDefault();track.style.transition='none';track.style.transform='translateX(-'+(getOffset()-dx)+'px)';},{passive:false});
    viewport.addEventListener('touchend',function(e){if(!isDragging)return;isDragging=false;var dx=e.changedTouches[0].clientX-touchStartX;if(Math.abs(dx)>50){goTo(dx<0?1:-1);}else{applyTranslate(true);}});
    window.addEventListener('resize',function(){initSizes();applyTranslate(false);});
})();
</script>
...
(function () {
    ...
    function animateCounters() {
        ...
        block.querySelectorAll('.js-stat-number').forEach(function(el) {
            var target=parseFloat(el.dataset.target)||0, duration=1500, start=null;
            function step(ts){if(!start)start=ts;var progress=Math.min((ts-start)/duration,1),ease=1-Math.pow(1-progress,3);el.textContent=Math.round(ease*target).toLocaleString();if(progress<1)requestAnimationFrame(step);}
            requestAnimationFrame(step);
        });
    }
    var observer = new IntersectionObserver(function(entries){entries.forEach(function(e){if(e.isIntersecting)animateCounters();});},{threshold:0.3});
